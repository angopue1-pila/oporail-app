...
function startCustomTest() {
  ...
  if (cat !== 'Todas') {
    if (cat === 'Psicotécnicos') {
      query = query.in('categoria', ['Numérico', 'Verbal', 'Series de Figuras', 'Atención']);
    } else {
...
function answer(letra) {
  var q = currentTest.questions[currentTest.index];
  var correcta = (q.respuesta_correcta || '').trim().toUpperCase();
  var isBlank = letra === 'BLANK';
  var esCorrecta = isBlank ? false : letra.trim().toUpperCase() === correcta;
  ...
  if (currentTest.survival) {
    ...
    btns.forEach(function(b) {
      b.disabled = true;
      if (b.textContent[0].trim().toUpperCase() === correcta) {
        b.style.borderColor = '#16a34a'; 
        b.style.background = '#f0fdf4';
        b.style.color = '#166534';
      } else if (b.textContent[0].trim().toUpperCase() === letra.trim().toUpperCase() && !esCorrecta) {
    ...
    btns.forEach(function(b) {
      ...
      if (b.textContent[0].trim().toUpperCase() === correcta) {
        ...
        b.style.background = '#f0fdf4';
        b.style.color = '#166534';
      } else if (b.textContent[0].trim().toUpperCase() === letra.trim().toUpperCase() && !esCorrecta && !isBlank) {
        b.style.borderColor = '#ef4444'; 
        b.style.background = '#fef2f2';
        b.style.color = '#991b1b';
