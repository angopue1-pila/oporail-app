...
function showProfileScreen() {
  ...
  updateSyncStatus();
  renderProfileList();
  document.getElementById('back-to-app').style.display = currentProfile ? 'block' : 'none';
...
async function renderProfileList() {
  ...
  if (profiles.length === 0) {
    list.innerHTML = '<p class="muted">Todavía no hay perfiles.</p>';
    return;
  }
  profiles.sort();
  var html = '';
  profiles.forEach(function(profile) {
    html += '<button class="profile-choice" onclick="selectProfile(&quot;' + encodeURIComponent(profile) + '&quot;)">' + profile + '</button>';
  });
  list.innerHTML = html;
}
  ...
  if (currentTest.survival) {
    ...
    btns.forEach(function(b) {
      ...
      if (b.textContent[0] === correcta) {
        ...
        b.style.borderColor = '#ef4444'; b.style.background = '#fef2f2';
