...
function answer(letra) {
  ...
  if (currentTest.survival) {
    ...
    if (!esCorrecta) {
      ...
      }, 2000); // 2 segundos para ver la respuesta correcta
      return;
    }
...
function showCurrentMistakes() {
  ...
  var html = '<h2 style="margin-bottom:20px;">Detalle de Fallos</h2>';
  ...
  if (mistakes.length === 0) {
    ...
    mistakes.forEach(function(m, i) {
      html += '<div class="card" style="background: rgba(239, 68, 68, 0.05); margin-bottom: 15px; border-left: 4px solid #ef4444; text-align: left;">' +
        '<p style="font-size: 13px; color: var(--muted); margin-bottom: 5px;">Pregunta ' + (i + 1) + '</p>' +
        '<p style="margin-bottom: 10px;"><strong>' + m.pregunta + '</strong></p>' +
        '<p style="color: #ef4444; font-weight: 600; margin-bottom: 5px;">Tu respuesta: ' + m.respuesta_usuario + '</p>' +
        '<p style="color: #16a34a; font-weight: 600; margin-bottom: 10px;">Respuesta correcta: ' + m.respuesta_correcta + '</p>' +
...
function showPastTestDetails(id) {
  ...
  var html = `<h3>${test.tipo_test}</h3>
    <p class="muted">${new Date(test.fecha).toLocaleString()}</p>
    <div style="font-size: 24px; font-weight: 800; text-align: center; margin: 20px 0;">Nota: ${nota}</div>
  ...
  test.detalles.forEach(function(m, i) {
    ...
    html += `<div class="card" style="background: ${bgColor}; margin-bottom: 15px; border-left: 4px solid ${borderColor}; text-align: left;">
      ...
      <p style="margin-bottom: 10px;"><strong>${m.pregunta}</strong></p>
      <p style="color: ${isCorrect ? '#16a34a' : '#ef4444'}; font-weight: 600; margin-bottom: 5px;">Tu respuesta: ${m.respuesta_usuario}</p>
      ${!isCorrect ? `<p style="color: #16a34a; font-weight: 600; margin-bottom: 10px;">Respuesta correcta: ${m.respuesta_correcta}</p>` : ''}
      ${m.explicacion ? `<div style="background: var(--surface-soft); padding: 10px; border-radius: 8px; font-size: 13px; color: var(--text-soft);">💡 ${m.explicacion}</div>` : ''}
    </div>`;
  });
