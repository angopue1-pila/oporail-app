...
function startWordGame() {
  ...
  var record = getRecord('words');
  var recordEl = document.getElementById('game-record-words');
...
function guessLetter(letter) {
  ...
  if (normalizedWord.indexOf(normalizedInput) === -1) {
    ...
    if (wordGameState.mistakes >= 3) {
      setTimeout(function() {
        var isNewRecord = updateRecord('words', wordGameState.points);
        if (isNewRecord) {
          confetti({
            particleCount: 150,
            spread: 70,
          ...
          alert('💥 ¡HAS PERDIDO!\nLa palabra era: ' + wordGameState.currentWord + '\n\n🏆 ¡NUEVO RÉCORD!: ' + wordGameState.points + ' pts');
        } else {
          alert('💥 ¡HAS PERDIDO! La palabra era: ' + wordGameState.currentWord + '\nPuntos: ' + wordGameState.points);
        }
        backHome();
      }, 300);
      return;
